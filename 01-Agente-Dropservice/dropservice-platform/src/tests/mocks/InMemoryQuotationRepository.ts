...
export class InMemoryQuotationRepository implements IQuotationRepository {
  ...
  async findLatestByOrderId(orderId: UniqueEntityID): Promise<Result<Quotation | null, string>> {
  ...
  async getClientEmail(_clientId: string): Promise<Result<string | null, string>> {
    return ok('test-client@example.com');
  }

  async update(quotation: Quotation): Promise<Result<void, string>> {
    return this.save(quotation);
  }

  async findByOrderId(orderId: UniqueEntityID): Promise<Result<Quotation[], string>> {
    return this.findByOrder(orderId);
  }

  async findByProviderId(providerId: UniqueEntityID): Promise<Result<Quotation[], string>> {
