...
|----------|---------|----------|
| Node.js | `telegraf` | Most projects (Standard) |
| Node.js | `grammY` | TypeScript, modern features |
...
**Why bad:** Telegram has timeout limits. Users think bot is dead. Poor experience.
**Instead:** Acknowledge immediately. Process in background. Send update when done. Use typing indicator (`ctx.sendChatAction('typing')`).

### ❌ No Error Handling

**Why bad:** Users get no response. Bot appears broken. Debugging nightmare.
**Instead:** Global error handler. Graceful error messages to user. Log errors for debugging.

### ❌ Spammy Bot

**Why bad:** Users block the bot. Telegram may ban. Low retention.
